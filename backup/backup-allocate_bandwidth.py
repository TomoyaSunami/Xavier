...
def main(argv):
    ...
    while(1):
        ...
        print("count: {},  iperf_on_off: {},  iperf_start: {},  iperf_kill: {}".format(count, iperf_state, iperf_start, iperf_kill))
    ...
    while key != 113:  # for 'q' key
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            #cv2.imwrite("output/image{0:03d}.jpg".format(save_image_num), image)
            #save_image_num += 1
            """ 
            labels = [detection[0] for detection in detections]
            if ("person" in labels) and hit_count < 10:
                hit_count += 1
            elif ("person" not in labels) and hit_count > 0 :
                hit_count -= 1 
            if iperf_on_off == "OFF" and hit_count > 5:
                future = executor.submit(start_iperf)
                iperf_on_off = "ON"
                log.info("start iperf")
            elif iperf_on_off == "ON" and hit_count < 2:
