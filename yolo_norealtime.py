...
def plot_bounding_box(image, detections, color_array, depth):
...
def execute_cmdline(argv):
    ...
    loaunch_yolo_env(config_path, weight_path, meta_path, thresh, svo_path, zed_id)

def loaunch_yolo_env(config_path, weight_path, meta_path, thresh, svo_path, zed_id):
...
def main(cam, runtime, mat, point_cloud_mat, thresh, color_array):
    ...
    while key != 113:  # for 'q' key
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            with open("yolo_output.txt", mode='a') as f:
                str_w = str(timestamp)+","+str(confidence)+ "\n"
                f.write(str_w)
